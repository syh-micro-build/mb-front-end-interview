...
* **1：** `<Nuxt>`Nuxt 框架中的核心组件、`<NuxtLink>`用于在 Nuxt 应用内部进行页面导航、`<ClientOnly>`只在客户端被渲染和执行、
`<NoSSR>`用来处理不适合在服务端渲染的内容、`<Transition>`用于给页面或组件的切换添加过渡动画效果，增强用户体验，让页面之间的转换更加平滑自然。可以设置不同
的动画类型、时长、延迟等参数。
* **1：** `<NuxtChild>`主要用于嵌套路由场景。当有多层级的路由，且子路由对应的页面需要在父路由布局中的特定位置展示时，就会用到该组件、
`<KeepAlive>`保留组件的状态，让它在切换后依然保持之前的状态、`<slot>`用于实现内容分发机制。父组件可以向子组件传递内容，子组件通过slot来接收并展示这些内容.